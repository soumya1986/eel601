...
public class MatrixMultiplicationOnRing2 {
	...
	private static void distributeFirstMatrix(){
		if(rank == 0) { 
			 ...
			 Matrix[] matrixObjArr_A = new Matrix[1];
			 matrixObjArr_A[0] = new Matrix(matrix_A);
			 //display2DMatrix(matrix_A);
			 
			 getMyElementsOfMatrix(matrix_A, rank);
			 
			 peer = 1;
             MPI.COMM_WORLD.Ssend(matrixObjArr_A, 0, 1, MPI.OBJECT, peer, tag) ; 
        	 ...
        	 Matrix[] matrixObjArr_A = new Matrix[1];
        	 peer = 0;
        	 status = MPI.COMM_WORLD.Recv(matrixObjArr_A, 0, 1, MPI.OBJECT, peer, tag); 
        	 
        	 getMyElementsOfMatrix(matrixObjArr_A[0].getArray2D(), rank);
	...
	private static void distributeSecondMatrix(int index){
		if(rank == 0) { 
			...
			Matrix[] matrixObjArr_B_sub = new Matrix[1];
			 ...
			 */
			getSpecificColumnsOfMatrix(matrix_B, index, factor);
			
			
			matrixObjArr_B_sub[0] = new Matrix(myElementsOfMatrixB_2D);
			 
			 peer = 1;
             MPI.COMM_WORLD.Ssend(matrixObjArr_B_sub, 0, 1, MPI.OBJECT, peer, tag) ; 
             ...
             MPI.COMM_WORLD.Recv(receivedMatrix_1D, 0, 1, MPI.INT, peer, peer); 
             ...
             */
        	 ...
        	 peer = 0;
        	 status = MPI.COMM_WORLD.Recv(matrixObjArr_B_sub, 0, 1, MPI.OBJECT, peer, tag); 
	...
	private static void getMyElementsOfMatrix(int[][] matrix_2D, int rank){
		
		for(int i=0;i<factor;i++){
			myElementsOfMatrixA_2D[i] = matrix_2D[rank*factor + i];
		}
	...
	private static void getSpecificColumnsOfMatrix(int[][] matrix_2D, int index, int factor){
		//display2DMatrix(matrix_2D);
		for(int i=0;i<numOfRowsOrCols;i++){
			for(int j=0;j<factor;j++){
				myElementsOfMatrixB_2D[i][j] = matrix_2D[i][index*factor + j];
			}
		}
